public class Solution {
    ...
    public boolean isNumber(String s) {
        ...
        if(e){
            if(dot) return dot1<e1&&isFloat(start, e1-start, dot1)&&(e1+esign!=chars.length-1&&isInteger(e1+esign+1, chars.length-e1-esign-1));
            else return (e1!=start&&isInteger(start, e1-start))&&(e1+esign!=chars.length-1&&isInteger(e1+esign+1, chars.length-e1-esign-1));
